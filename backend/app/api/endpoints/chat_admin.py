...
def cap_nhat_ton_kho(ma_sanpham: int, so_luong_moi: int):
    """
    Cập nhật số lượng tồn kho mới cho một sản phẩm cụ thể.
    - ma_sanpham: ID của sản phẩm (ma_sanpham)
    - so_luong_moi: Số lượng tồn kho mới cần cập nhật
    """
    db = SessionLocal()
    try:
        product = db.query(Sanpham).filter(Sanpham.ma_sanpham == ma_sanpham).first()
        if not product:
            return f"Không tìm thấy sản phẩm có ID {ma_sanpham}."
        ...
        return {
            "tin_nhan": f"Đã cập nhật tồn kho cho '{product.ten_sanpham}' thành công.",
            "id": product.ma_sanpham,
            "ton_kho_cu": old_val,
            "ton_kho_moi": so_luong_moi
        }
    except Exception as e:
        db.rollback()
        return f"Lỗi: {str(e)}"
...
- cap_nhat_ton_kho(ma_sanpham, so_luong_moi): Cập nhật số lượng tồn kho cho sản phẩm
...
🔄 QUY TRÌNH CẬP NHẬT KHO:
1. Nếu admin yêu cầu cập nhật theo TÊN:
   - Gọi `tra_cuu_du_lieu(loai='sanpham', tu_khoa='...')` để lấy ID.
   - Gọi `cap_nhat_ton_kho(ma_sanpham, so_luong_moi)`.
2. Nếu admin yêu cầu tìm sản phẩm HẾT HÀNG để cập nhật:
   - Gọi `kiem_tra_ton_kho(nguong=0)` hoặc ngưỡng phù hợp.
   - Trình bày danh sách cho admin và hỏi số lượng cần cập nhật cho từng món hoặc tất cả.
   - Thực hiện gọi `cap_nhat_ton_kho` cho từng ID mà admin yêu cầu.
...
def chat_with_admin_ai(item: ChatRequest, db: Session = Depends(get_db), admin: User = Depends(check_admin_role)):
    ...
    # --- TỰ ĐỘNG DỌN DẸP LỊCH SỬ ĐỂ TRÁNH ĐẦY DATABASE ---
    # Chỉ giữ lại 100 tin nhắn gần nhất cho mỗi Admin
    try:
        excess_messages = db.query(LichSuChat).filter(
            LichSuChat.user_id == admin.ma_user,
            LichSuChat.context_type == "admin_ai"
        ).order_by(LichSuChat.thoi_gian.desc()).offset(100).all()
        
        for msg in excess_messages:
            db.delete(msg)
        db.commit()
    except Exception as e:
        print(f"⚠️ Warning: Lỗi khi dọn dẹp lịch sử chat: {e}")
        db.rollback()

    reply = ""
    try:
        # 2. Lấy lịch sử chat gần đây (ví dụ 10 tin nhắn cuối) để duy trì ngữ cảnh
        ...
        # Chuyển đổi sang định dạng của Gemini (user/model)
        gemini_history = []
        for m in history_msgs:
            role = "user" if m.role == "user" else "model"
            gemini_history.append({"role": role, "parts": [m.message]})

        # 3. Gọi AI (Gemini 2.0 Flash với tools và history)
        if model:
